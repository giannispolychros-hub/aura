...
export default function AURAv2() {
  ...
  const generateResponse = useCallback(async (msgs, currentMode) => {
    ...
    try {
      ...
      // RT-hardening: passive observation only — logs potential advice-cascade pattern for evidence
      // gathering, does not alter behavior. Decision on whether to act on this awaits real-transcript evidence.
      if (currentMode === "ANSWER" && looksLikeAdviceCascade(text)) {
        console.warn("[AURA watch] possible advice-cascade pattern detected (observational only):", text.slice(0, 80));
      }
