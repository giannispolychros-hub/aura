...
function detectAssistantSelfRepetition(messages) {
  ...
  const lexicalSim = [...a].filter(w => b.has(w)).length / Math.max(a.size, b.size, 1);
  // Opening-phrase check catches "discourse move" repetition the word-overlap check alone
  // would miss (e.g. always starting with "Τι σε κάνει..." even with different words after).
  const openingWords = s => (s.trim().toLowerCase().match(/^[a-zα-ωάέήίόύώϊϋΐΰ]+(\s+[a-zα-ωάέήίόύώϊϋΐΰ]+)?/) || [""])[0];
  const sameOpening = openingWords(last) && openingWords(last) === openingWords(prev);
  if (lexicalSim > 0.35 || sameOpening) return { repeated: true, lexicalSim, sameOpening };
